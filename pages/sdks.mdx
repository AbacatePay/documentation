...
// Crie uma cobrança aceitando PIX e cartão (método CARD em beta)
async function createBilling() {
  const billing = await abacate.billing.create({
    frequency: "ONE_TIME",
    methods: ["PIX", "CARD"],  // aceita um ou mais métodos
    products: [
      {
        externalId: "PRO-PLAN",
        name: "Pro plan",
...
# Crie uma cobrança (aceita PIX e/ou CARD; método CARD em beta)
