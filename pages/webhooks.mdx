...
Além do `webhookSecret`, você pode verificar a integridade da mensagem conferindo a assinatura enviada no cabeçalho `X-Webhook-Signature`. O exemplo abaixo calcula a assinatura HMAC-SHA256 sobre o corpo bruto da requisição e compara de forma segura com o valor recebido.
...
Nota: para que a verificação funcione, é essencial ler o corpo bruto da requisição (sem transformações) antes de qualquer middleware que o parseie. Garanta que você capture os bytes exatos recebidos ao calcular o HMAC.
