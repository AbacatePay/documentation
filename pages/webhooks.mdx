...
3. **Sempre que algo importante acontece**, como um pagamento aprovado:
   ...
   - O POST contém o evento (ex: `checkout.completed`, `payment.completed`)
...
| `payment.refunded` | Reembolso de um pagamento foi concluído |
...
<Tabs>
  <Tab title="payment.completed">
    Disparado quando um pagamento é confirmado independente se foi realizado via checkout ou via checkout transparente. O `payerInformation` varia conforme o método.
  ...
  <Tab title="payment.disputed">
    Disparado quando um pagamento sofre alguma disputa (checkout ou transparente). O `payerInformation` varia conforme o método.
  ...
  <Tab title="payment.refunded">
    Disparado quando um pagamento é reembolsado após um pagamento ser reembolsado (checkout ou transparente). O `payerInformation` varia conforme o método.
