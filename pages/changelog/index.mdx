...
<Update label="23 de Set, 2026">
## Assinaturas: checkout e assinatura com rotas próprias

O checkout de assinatura (`bill_...`, criado pelo `POST /v2/subscriptions/create`) saía pelo `GET /v2/checkouts/get`, mas por nenhuma listagem: o `GET /v2/checkouts/list` só lista checkouts de pagamento único, e o `GET /v2/subscriptions/list` lista as assinaturas (`subs_...`), não os checkouts. Agora cada recurso tem rota própria, e a assinatura aponta o checkout que a originou.

**Novidades:**

- Novo [`GET /v2/subscriptions/checkouts/get`](/pages/subscriptions/checkouts-get) e [`GET /v2/subscriptions/checkouts/list`](/pages/subscriptions/checkouts-list): buscam e listam checkouts de assinatura, pagos ou não, no mesmo formato do `/v2/checkouts`. Requerem `SUBSCRIPTION:READ`
- Novo [`POST /v2/subscriptions/checkouts/delete`](/pages/subscriptions/checkouts-delete): exclui um checkout de assinatura. Requer `SUBSCRIPTION:DELETE`. As assinaturas que ele já gerou continuam ativas
- Novo campo `checkoutId` nas respostas de `GET /v2/subscriptions/get`, `GET /v2/subscriptions/list` e `POST /v2/subscriptions/cancel`
- Novo filtro `checkoutId` no `GET /v2/subscriptions/list`. Um checkout pode gerar várias assinaturas (uma por cliente que o pagou), e um `checkoutId` inexistente retorna lista vazia

<Warning>
  **Mudança incompatível:** `GET /v2/checkouts/get` e `GET /v2/payment-links/get` **não retornam mais checkouts de assinatura**. Buscar um `bill_...` de assinatura por essas rotas retorna `This is a subscription checkout. Use /v2/subscriptions/checkouts/get`. Troque a chamada por [`GET /v2/subscriptions/checkouts/get`](/pages/subscriptions/checkouts-get); os parâmetros e o formato da resposta são os mesmos.

  Da mesma forma, `POST /v2/checkouts/delete` e `POST /v2/payment-links/delete` **não excluem mais checkouts de assinatura**. Use [`POST /v2/subscriptions/checkouts/delete`](/pages/subscriptions/checkouts-delete).
</Warning>

**Correções:**

- `GET /v2/subscriptions/get`: os filtros `externalId` e `customerId` não funcionavam. Agora `externalId` busca pelo `externalId` do checkout de assinatura que originou a assinatura, e `customerId` aceita o `cust_...`. O parâmetro `method` foi removido, porque não se aplica à assinatura. Quando o filtro casa mais de uma assinatura, a mais recente é retornada
- `GET /v2/checkouts/get`, `GET /v2/payment-links/get` e `GET /v2/subscriptions/get` exigem ao menos um entre `id`, `externalId` e `customerId`. Antes, sem nenhum deles, a rota retornava um registro qualquer da loja; agora retorna `Provide id, externalId or customerId`
- Um `customerId` inexistente no get e no list de checkouts retornava erro 500. Agora retorna `Billing not found` no get e lista vazia no list
- As respostas do get de checkouts, links de pagamento, assinaturas e checkouts transparentes (`GET /v2/transparents/get`) ficavam em cache por até 1 hora, então o `status` podia aparecer desatualizado logo depois de um pagamento ou cancelamento. O cache agora dura 60 segundos, igual ao das listagens
- Filtro `method` no get e no list de checkouts, links de pagamento e checkouts de assinatura: `method=PIX` só trazia checkouts que ofereciam **apenas** PIX. Agora traz todo checkout que oferece PIX, mesmo que ofereça outros métodos
- Reembolso de cobrança de assinatura: `POST /v2/checkouts/refund` mandava usar `/v2/payment-links/refund`, e o `POST /v2/payment-links/refund` mandava usar `/v2/checkouts/refund`. As duas rotas agora retornam `Cobranças de assinatura não podem ser reembolsadas pela API v2.`

**Exemplo:**

```http
# Checkouts de assinatura criados em agosto
GET /v2/subscriptions/checkouts/list?startDate=2026-08-01&endDate=2026-08-31

# Assinaturas geradas por um checkout
...
Consulte a [referência de assinaturas](/pages/subscriptions/reference) para o modelo completo.

---

</Update>

<Update label="18 de Set, 2026">
## Webhooks: retentativas automáticas

Quando seu endpoint de webhook fica fora do ar ou responde com erro temporário, a AbacatePay agora **reenvia o evento automaticamente** até ele ser entregue.

**O que mudou:**

- Falhas temporárias são retentadas em **5s, 5min, 30min, 2h, 5h e 10h** — até 7 tentativas em cerca de 18 horas
- Geram retentativa: timeout de **30 segundos**, erro de conexão/DNS/TLS, `5xx`, `408` e `429`
- Não geram retentativa: `3xx` e os demais `4xx` (ex.: `400`, `401`, `404`). `410 Gone` continua desativando o webhook
- **Todas as tentativas de um evento chegam com o mesmo `id`** no corpo, para você ignorar duplicatas
- Cada tentativa aparece nos logs do webhook no dashboard
...
<Warning>
  **Ação recomendada:** se seu endpoint ainda não trata eventos repetidos, passe a guardar o `id` de cada evento processado e responda `200 OK` sem reprocessar quando ele chegar de novo. Sem isso, um endpoint que voltou do ar pode processar o mesmo pagamento mais de uma vez.
</Warning>
...
Consulte [Entrega e retentativas](/pages/webhooks#entrega-e-retentativas) para detalhes completos.

---

</Update>

<Update label="11 de Set, 2026">
## PIX transparente: travar o pagador (`ensureSameTaxId`)

Agora é possível exigir que o PIX seja pago pelo **mesmo CPF/CNPJ** informado no cliente da cobrança. Se outra pessoa tentar pagar o QR Code, o pagamento é recusado.

**O que mudou:**

- Novo campo opcional `ensureSameTaxId` (`boolean`, padrão `false`) em `POST /v2/transparents/create` com `method: "PIX"`
- Exige `customer` com `taxId`; sem isso a API retorna `ensureSameTaxId requires a customer with taxId`
- Depende do provedor de PIX da conta — se o provedor ativo não suportar a trava, a API retorna `ensureSameTaxId is not supported by the current PIX provider`
- Ignorado em boleto e não aplicado em `devMode`
...
Consulte [Criar cobrança PIX](/pages/transparents/create) para detalhes completos.

---

</Update>

<Update label="4 de Ago, 2026">
## Listagem: filtro por data (`startDate` / `endDate`)

Todos os endpoints de listagem da API v2 agora suportam filtro por intervalo de datas de criação, além da paginação por cursor já existente.

**O que mudou:**

- Novos query params opcionais `startDate` e `endDate` (`YYYY-MM-DD`) em todas as rotas `GET /v2/*/list`
- Compatível com `limit`, `after` e `before` — paginação e filtro de data podem ser usados juntos
- As datas são interpretadas no fuso horário `America/Sao_Paulo` (início do dia para `startDate`, fim do dia para `endDate`)
- Validações aplicadas automaticamente:
  - `startDate` não pode ser posterior a `endDate`
  - Nenhuma data pode ser anterior a `2024-01-01`
  - Nenhuma data pode ser superior a **1 ano no futuro** em relação à data atual

**Endpoints afetados:**

Clientes, cupons, produtos, checkouts, links de pagamento, assinaturas, webhooks, saques, PIX enviados, checkouts transparentes e demais rotas `*/list` da v2.
...
Consulte [Paginação e filtro por data](/pages/reference/introduction#paginação-e-filtro-por-data) para detalhes completos.
