---
title: 'Criar Checkout de assinatura'
openapi: "POST /subscriptions/create"
---

Cria um checkout de assinatura. O cliente paga uma vez e entra no ciclo de cobranças recorrentes do produto.

<Card title="Pré-requisito" horizontal>
  O produto referenciado em `items` deve ter um `cycle` definido (`WEEKLY`, `MONTHLY`, `SEMIANNUALLY` ou `ANNUALLY`). Produtos avulsos retornam erro.
</Card>

**Exemplo:**
```json
{
  "items": [
    { "id": "prod_abc123xyz", "quantity": 1 }
  ],
  "customerId": "cust_abc123xyz",
  "externalId": "subs-123",
  "completionUrl": "https://seusite.com/sucesso",
  "methods": ["CARD"]
}
```

<Tip title="Use a url retornada" horizontal>
Redirecione o cliente para `data.url` para concluir o primeiro pagamento e ativar a assinatura.
</Tip>

<Note>
Assinaturas aceitam apenas **um produto por checkout**. O método padrão é `CARD`.
</Note>

<Note title="Período de teste gratuito" horizontal>
Se o produto referenciado tiver `trialDays` configurado, o checkout cobra **R$ 0,00** — apenas tokeniza o cartão. A primeira cobrança pelo valor integral ocorre automaticamente ao final do trial. Consulte a [referência de assinaturas](/pages/subscriptions/reference#período-de-teste-gratuito-free-trial) para mais detalhes.
</Note>
