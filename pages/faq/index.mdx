...
description: 'Respostas para as dúvidas mais comuns sobre a AbacatePay — para lojistas e desenvolvedores.'
icon: 'circle-question'
---

### Para lojistas (sem código)

<AccordionGroup>
  <Accordion title="Preciso saber programar para usar a AbacatePay?">
    Não. Você pode criar cobranças, gerar links de pagamento e gerenciar tudo pelo dashboard sem escrever uma linha de código. A API e os SDKs são para quem quer automatizar o processo dentro de um sistema.

    Veja o guia [Receba seu primeiro pagamento](/pages/guides/primeiro-pagamento) para começar sem código.
  </Accordion>

  <Accordion title="Como recebo meu dinheiro?">
    O dinheiro dos pagamentos fica no seu saldo na AbacatePay. Você faz um saque a qualquer momento pelo dashboard para transferir para sua conta bancária. PIX cai em segundos, cartão tem um prazo de liquidação. Veja mais em [Como receber seu dinheiro](/pages/concepts/saques).
  </Accordion>

  <Accordion title="Posso cobrar pelo WhatsApp ou Instagram?">
    Sim. Você cria um link de pagamento no dashboard e compartilha esse link em qualquer canal — WhatsApp, Instagram, e-mail, SMS. O cliente clica, escolhe a forma de pagamento e pronto.
  </Accordion>

  <Accordion title="Quais formas de pagamento estão disponíveis?">
    PIX, cartão de crédito (com parcelamento em até 12x) e boleto bancário. Você pode habilitar ou desabilitar cada forma ao criar uma cobrança.
  </Accordion>

  <Accordion title="Como faço para cobrar todo mês (mensalidade)?">
    Crie um produto com recorrência no dashboard. Seu cliente paga uma vez e é cobrado automaticamente a cada ciclo (mensal, semanal ou anual). Veja [O que são Assinaturas](/pages/concepts/subscriptions).
  </Accordion>

  <Accordion title="Minha conta precisa ser verificada para receber pagamentos reais?">
    Sim. Você pode testar tudo no Dev Mode (ambiente de testes) sem verificação. Mas para processar pagamentos reais e sacar dinheiro, sua conta precisa estar verificada. O processo de verificação é feito no próprio dashboard.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="A AbacatePay tenta reenviar eventos se meu endpoint estiver fora do ar?">
    Sim. Se o endpoint não responder em 30 segundos, estiver fora do ar ou responder `5xx`, `408` ou `429`, a AbacatePay tenta de novo em 5s, 5min, 30min, 2h, 5h e 10h — até 7 tentativas em cerca de 18 horas. Outros `4xx` não são retentados. Todas as tentativas usam o mesmo `id` do evento: use-o para evitar processamento duplicado. Veja [Entrega e retentativas](/pages/webhooks#entrega-e-retentativas).
  </Accordion>
