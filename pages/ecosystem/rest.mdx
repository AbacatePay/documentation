...
Você não precisa fornecer um secret caso não queira, o @abacatepay/rest tenta pegar automáticamente do env `ABACATEPAY_SECRET` ou `ABACATEPAY_API_KEY`.
...
Você também pode controlar **completamente** quando um rate-limit acontece:
...
<Icon icon="boxes-stacked" type="solid"/> REST Client vs SDKs

<CardGroup cols={2}> <Card title="REST Client" icon="sparkles"> Camada fina, controle total, ideal para libs, CLIs e integrações avançadas. </Card> <Card title="SDKs" icon="boxes-stacked" href="../sdks"> Abstrações de alto nível para integração rápida com menos código. </Card> </CardGroup>

<Icon icon="book" type="solid"/> Próximos Passos
